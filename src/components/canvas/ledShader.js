...
export const ledVertexShader = /* glsl */ `
  ...
  uniform float uTime;
  uniform int uChapter;      // active chapter index (0..7)
  ...
  void main() {
    // Select Target A (current chapter) on the GPU
    vec3 targetA = aTarget0;
    if (uChapter == 1) targetA = aTarget1;
    else if (uChapter == 2) targetA = aTarget2;
    else if (uChapter == 3) targetA = aTarget3;
    else if (uChapter == 4) targetA = aTarget4;
    else if (uChapter == 5) targetA = aTarget5;
    else if (uChapter == 6) targetA = aTarget6;
    else if (uChapter == 7) targetA = aTarget7;

    // Select Target B (next chapter) on the GPU
    int nextChapter = uChapter + 1;
    if (nextChapter > 7) nextChapter = 7;
    
    vec3 targetB = aTarget0;
    if (nextChapter == 1) targetB = aTarget1;
    else if (nextChapter == 2) targetB = aTarget2;
    else if (nextChapter == 3) targetB = aTarget3;
    else if (nextChapter == 4) targetB = aTarget4;
    else if (nextChapter == 5) targetB = aTarget5;
    else if (nextChapter == 6) targetB = aTarget6;
    else if (nextChapter == 7) targetB = aTarget7;

    // Smoothly blend targetA -> targetB for chapter transitions
    vec3 target = mix(targetA, targetB, uMorphB);
